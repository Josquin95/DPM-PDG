...
#A1- Longitud
# Longitud de la url
def longitudURL(input_path):
    print "A1"
    #dataset=open("../recursos/dataset_1.txt", "r")
    #dataset = open(input_path)
    try:
        #for linea in input_path:
        print "Entra--------- "+ input_path
        lineS = input_path.split(';')
        print "Split--------- "+ lineS
        longitud = len(str(lineS[1]))
        print len(str(lineS[1]))
    except Exception:
        longitud = "-1"
        print "A1 Error en la URL " + input_path
...
def caracteresExtranos(input_path):
    print "A2"
    #dataset=open("../recursos/dataset_1.txt", "r")
    #dataset = open(input_path)
    try:
        ...
        lineS = input_path.split(';')
        contadorAlfabetico = sum(1 for c in lineS[1] if c.isalpha()) #cuantas letras hay por linea
        contadorNumero = sum(1 for c in lineS[1] if c.isdigit()) #cuantos numero hay por linea
        caracteresAlfabeticos = contadorNumero+contadorAlfabetico
        longitud = len(str(lineS[1]))#tamano de la linea
    ...
    with open(name=input_path, mode='r', buffering=1) as datasets:
         with open(name=ruta_matriz_app, mode='a+') as matrizApp:
            for linea in datasets:
                ...
                try:
                    line_id_url = linea.split(';')[0]
                    matrizApp.writelines(line_id_url + ';' 
                                        + longitudURL(linea) + ';' 
                                        + caracteresExtranos(linea) + '\n')
                                        #+ nombreDominio(linea) + ';' 
                    ...
                    print "Error al crear la matriz"
